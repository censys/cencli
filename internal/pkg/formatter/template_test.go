...
`
		...
		{
			name: "template with length helper",
			templatePath: func(t *testing.T, tempDir string) string {
				templatePath := filepath.Join(tempDir, "length_helper.hbs")
				templateContent := `{{#each teams}}
...
- All tags: {{length all_tags}}
- Config keys: {{length config}}`
				err := os.WriteFile(templatePath, []byte(templateContent), 0o644)
				require.NoError(t, err)
				...
				return map[string]interface{}{
					"teams": []Team{
						...
						},
						{
							...
							Description: "Frontend development team responsible for user interfaces",
						},
					},
					"all_tags": []string{"backend", "api", "database", "frontend", "ui", "testing"},
  ...
  - Charlie: 0 tasks
Total tags: 3
Description length: 24 characters
---
